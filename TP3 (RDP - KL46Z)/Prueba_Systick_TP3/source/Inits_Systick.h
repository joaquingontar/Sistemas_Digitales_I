...
void init_clock_MCGIRCLK(){
	...
	MCG->SC=0x04; // Utilizo el divisor de 4 para obtener un clock de 1Mhz (ciclo de 1us)
}
...
void SysTick_Init(void) {
	if (contando==0){
		SysTick->LOAD = 2000000 - 1;       // Cargar para contar 1 ms (1000 ciclos a 1 MHz)
		SysTick->VAL = 0;
		SysTick->CTRL = 5;              // Habilitar SysTick, interrupciones y seleccionar el reloj del sistema
	}
}
...
void SysTick_Poll(void) {
	if (SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk) { // Verifica si el contador llegó a 0
			Ft2 = 1;              // Establece la bandera indicando que pasaron 2 segundos
		}
	}
